...
func main() {
	...
	switch action {
	case "init":
		createTables(conn138)
	case "loaddata":
		loadDataFromTable(conn138)
	case "cleanup":
		dropTables(conn138)
	default:
		sumRequestNumByUrl(conn138)
	}
}
...
func loadDataFromTable(conn *sqlx.DB) {
	conn.MustExec(`INSERT INTO tutorial.hits_ddl_all SELECT * FROM tutorial.hits_v1`)
}

func createTables(conn *sqlx.DB) {
	conn.MustExec(createDBSchema)
	conn.MustExec(createLocalTableSchema)
	conn.MustExec(createAllTableSchema)
}

func dropTables(conn *sqlx.DB) {
	conn.MustExec(dropLocalTableSchema)
	conn.MustExec(dropAllTableSchema)
}

var dropLocalTableSchema = `
DROP TABLE tutorial.hits_ddl_local ON CLUSTER tm_cluster_one_shard_two_replicas
`

var dropAllTableSchema = `
DROP TABLE tutorial.hits_ddl_all ON CLUSTER tm_cluster_one_shard_two_replicas
`

var createDBSchema = `
CREATE DATABASE IF NOT EXISTS tutorial ON CLUSTER tm_cluster_one_shard_two_replicas
`

var createLocalTableSchema = `
...
var createAllTableSchema = `
CREATE TABLE IF NOT EXISTS tutorial.hits_ddl_all ON CLUSTER tm_cluster_one_shard_two_replicas AS tutorial.hits_ddl_local
ENGINE = Distributed(tm_cluster_one_shard_two_replicas, tutorial, hits_ddl_local, rand())
`
